...
export default function AgentPerformance() {
  ...
  useEffect(()=>{fetch("/api/agent/performance").then(r=>{if(r.status===401){router.replace("/portal/login");return null;}return r.ok?r.json():Promise.reject();}).then(body=>{if(body){setData(body.analytics);setCredits(body.creditsAvailable);}}).catch(()=>setData(null));},[router]);
...
function Empty({text}:{text:string}) { return <p className="rounded-xl border border-dashed border-[#d9cfbd] bg-[#faf8f3] p-3 text-xs leading-5 text-[#776e61]">{text}</p>; }
function Breakdown({label,values}:{label:string;values?:Record<string,number>}) { const rows=Object.entries(values??{}).sort((a,b)=>b[1]-a[1]).slice(0,4); return <div className="mb-4"><h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-[#776e61]">{label}</h3>{rows.length?rows.map(([name,count])=><div key={name} className="mb-2 flex justify-between rounded-lg bg-[#f7f3eb] px-3 py-2 text-sm"><span>{name}</span><strong>{count}</strong></div>):<Empty text="No attributed data is available yet."/>}</div>; }
