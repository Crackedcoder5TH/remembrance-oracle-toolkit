...
export async function GET(req: NextRequest,{params}:{params:{id:string}}){const auth=await authorize(req,params.id);if(auth instanceof NextResponse)return auth;return NextResponse.json({success:true,operations:await getLeadOperations(params.id,{clientId:auth.clientId})});}
export async function PATCH(req: NextRequest,{params}:{params:{id:string}}){
  ...
  const operations=await updateLeadOperations(params.id,auth.clientId,"agent",body,{clientId:auth.clientId});
