...
export async function PATCH(req: NextRequest,{params}:{params:{id:string}}){const auth=await authorize(req,params.id);if(auth instanceof NextResponse)return auth;const body=await req.json();if(body.status&&!AGENT_STATUSES.includes(body.status))return NextResponse.json({success:false,message:"Invalid status."},{status:400});for(const key of ["nextFollowUpAt","appointmentAt"]){if(body[key]&&Number.isNaN(Date.parse(body[key])))return NextResponse.json({success:false,message:`Invalid ${key}.`},{status:400});}if(body.note&&(!String(body.note).trim()||String(body.note).length>4000))return NextResponse.json({success:false,message:"Note must be between 1 and 4,000 characters."},{status:400});const operations=await updateLeadOperations(params.id,auth.clientId,"agent",body);if(body.status==="Do Not Contact"){const lead=await getLeadById(params.id);if(lead.ok&&lead.value){await Promise.all([addSuppression("phone",lead.value.phone,"Consumer Do Not Contact request","authorized agent report",auth.clientId),addSuppression("email",lead.value.email,"Consumer Do Not Contact request","authorized agent report",auth.clientId)]);}}await recordAudit({actorId:auth.clientId,actorRole:"agent",eventType:body.status==="Do Not Contact"?"do_not_contact_marked":body.eventType||"lead_operations_updated",targetType:"lead",targetId:params.id,summary:body.status==="Do Not Contact"?"Do Not Contact reported":"Authorized lead operations updated",ip:req.headers.get("x-forwarded-for")?.split(",")[0]?.trim()||null,userAgent:req.headers.get("user-agent")});return NextResponse.json({success:true,operations});}
