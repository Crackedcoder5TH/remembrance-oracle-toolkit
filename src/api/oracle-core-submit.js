...
// Field-validated growth: every ACCEPTED pattern now rides the two-oracle
// absorption gate (covenant-trust) — field projection AND shape validity
// must both say yes before the pattern feeds the recognized-shape
// registry; disagreement quarantines. This machinery was reachable only
// from a diagnostic script until 2026-08-08 (wire-later ledger, trap #24
// species). Best-effort: absorption never blocks an accept.
function _absorbAccepted(pattern, source) {
  try {
    const { maybeAbsorbPattern } = require('../core/covenant-trust');
...
module.exports = {
  ...
  submit(code, metadata = {}) {
    ...
    // Prove novel code. If no test was supplied, synthesize one offline
    // (signature-based, no API) so the validator actually RUNS the code and
    // records testPassed — which feeds coherency's testProof. The oracle thus
    // tests and proves any novel code it's given, not just code that arrives
    // with a test. Best-effort; opt out with metadata.autoProve: false.
    let effectiveTestCode = testCode;
    let testAutoGenerated = false;
    if (!effectiveTestCode && metadata.autoProve !== false) {
      try {
        const { synthesizeTests } = require('../evolution/test-synth');
        const langGuess = language || (/\bdef\s+\w+\s*\(/.test(code) ? 'python' : 'javascript');
        const synth = synthesizeTests(code, langGuess);
        if (synth && synth.trim()) { effectiveTestCode = synth; testAutoGenerated = true; }
      } catch (e) {
        if (process.env.ORACLE_DEBUG) console.warn('[oracle:submit] test synthesis failed:', e?.message || e);
      }
    }

    const validation = validateCode(code, {
      language, testCode: effectiveTestCode, threshold: this.threshold, description, tags, domain,
    });
    ...
    const entry = this.store.add({
      ...
      testCode: effectiveTestCode || null, testAutoGenerated,
    });
    // Native fractal search: keep the in-memory index in sync. Never
    // block submit on indexing — a failed add here just means this one
    ...
    const absorption = _absorbAccepted({ name: description || ('submit-' + entry.id), code, language: validation.coherencyScore.language, coherencyScore: validation.coherencyScore }, 'submit');
    return { success: true, accepted: true, entry, validation, similarity: simCheck.similarity, testAutoGenerated, absorption };
  },
  ...
  registerPattern(pattern) {
    ...
    const absorption = _absorbAccepted({ name: pattern.name, code: pattern.code, language: validation.coherencyScore.language, coherencyScore: validation.coherencyScore }, 'register');
    return { success: true, registered: true, pattern: registered, validation, growth: growthReport, quantum: quantumCapture, absorption };
  },
  ...
  evolvePattern(parentId, newCode, metadata = {}) {
    ...
    _absorbAccepted({ name: evolved.name, code: newCode, language: evolved.language, coherencyScore: evolved.coherencyScore }, 'evolve');
