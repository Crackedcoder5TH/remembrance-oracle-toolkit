  ...
  try {
    ...
    for (const __p of __lre_enginePaths) {
      try {
        ...
        __contribute({ cost: 1, coherence: Math.max(0, Math.min(1, __retVal.alignment === 'healing' ? 1 : __retVal.alignment === 'degrading' ? 0 : 0.5)), source: 'oracle:periodic-table:decodeSignature' });
        break;
      } catch (_) { /* try next */ }
    }
  } catch (_) { /* best-effort */ }
