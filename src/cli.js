...
${c.bold('Options:')}
  ...
  // ─── Lifecycle Engine CLI ───
  if (cmd === 'lifecycle') {
    const sub = args._command ? process.argv[3] : null;
...
${c.bold('Commands:')}
  ...
  // ─── Self-Improvement & Self-Optimization CLI ───
  if (cmd === 'improve') {
  ...
  if (cmd === 'full-cycle') {
    ...
    // Print the whisper summary
    if (report.whisper) {
      console.log(report.whisper);
    }

    // Print the whisper events summary if available
