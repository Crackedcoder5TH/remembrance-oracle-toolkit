...
/**
 * Lexicon Watcher — auto-detects new functions, terms, and architectural
 * emergence, queues them as lexicon proposals, and activates them when
 * coherency is in the semi-autonomous or autonomous range.
 *
 * Wiring:
 *   1. Subscribe a coherency stream: watcher.observe(score, { filePath, diff })
 *   2. On fluctuation > threshold, watcher.scanChanges() extracts new exports,
 *      new constant terms, and architectural shifts from the diff.
 *   3. Proposals land in .remembrance/lexicon-proposals.json with status pending.
 *   4. autoApproveIfCoherent() promotes pending -> active when sealCount() has
 *      risen OR rolling coherency >= 0.85 (synergy threshold).
 *
 * Runs from the post-commit hook, from CI, or on demand via:
 *   node src/core/lexicon-watcher.js --scan-head
 */
...
function detectFluctuation() {
  ...
  return {
    fluctuated: delta >= FLUCTUATION_THRESHOLD,
    delta: Math.round(delta * 1000) / 1000,
    current,
    mean: Math.round(mean * 1000) / 1000,
    direction: current > mean ? 'rising' : 'falling',
  };
}

function scanChanges(opts = {}) {
  const findings = { functions: [], terms: [], architectural: [] };
  let diffOutput;
  try {
  ...
  for (const file of files) {
    ...
    for (const m of fnMatches) {
      if (m[1] && !m[1].startsWith('_')) findings.functions.push({ name: m[1], file });
    }
...
function proposeLexiconEntries(findings, coherencyContext = {}) {
  ...
  for (const a of findings.architectural) {
    proposals.push({
      kind: 'architectural',
      name: `arch:${path.basename(a.file)}`,
      source: a.file,
      hint: a.hint,
      proposedAt: now,
      status: 'pending',
...
function autoApproveIfCoherent(currentCoherency) {
  ...
  return { promoted, totalActive: all.filter(p => p.status === 'active').length };
}
...
function runCycle({ coherency, ref }) {
  ...
  return { action: 'scanned', fluctuation, findings, proposals: proposals.length, persistResult, promotion, sealCount: SEAL_REGISTRY.length };
}
...
if (require.main === module) {
  ...
  if (arg === '--scan-head') {
    const result = runCycle({ ref: 'HEAD' });
    console.log(JSON.stringify(result, null, 2));
  } else {
    console.log('Usage: node lexicon-watcher.js --scan-head');
  }
}
...
module.exports = {
  ...
  detectFluctuation,
  scanChanges,
  proposeLexiconEntries,
  persist,
  autoApproveIfCoherent,
  runCycle,
