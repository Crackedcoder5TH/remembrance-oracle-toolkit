  ...
  // ─── 28. Remembrance Field (LRE) — the conserved ecosystem field, one tool ───
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        cost: { type: 'number', description: 'contribute / validate: work units consumed (default: 1).' },
        coherence: { description: 'contribute: alignment score (number, 0..1). validate: number for a single candidate, or number[] for a batch — the shape of the array is what is classified.' },
        source: { type: 'string', description: 'contribute / validate: caller identity, e.g. "agent:claude:my-task".' },
