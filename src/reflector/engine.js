...
function formatReport(report) {
  ...
  if (report.healings && report.healings.length > 0) {
    for (const h of report.healings) {
      lines.push(`  ${h.path}`);
      lines.push(`    ${(h.originalCoherence || 0).toFixed(3)} → ${(h.healedCoherence || 0).toFixed(3)} (+${(h.improvement || 0).toFixed(3)}) [${h.loops || 1} loop(s)]`);
  ...
  lines.push(`  Overall health: ${report.collectiveWhisper?.overallHealth || 'stable'}`);
...
function formatPRBody(report) {
  ...
  lines.push('');
  lines.push(`> ${report.collectiveWhisper?.message || 'No whisper available.'}`);
  lines.push('');
  ...
  if (report.healings && report.healings.length > 0) {
    ...
    for (const h of report.healings) {
      ...
      lines.push(`- **Strategy**: ${h.healingSummary || 'SERF refinement'}`);
      lines.push(`- **Whisper**: *"${h.whisper || ''}"*`);
      lines.push('');
    }
  }
  ...
  lines.push('');
  lines.push(`Overall codebase health: **${report.collectiveWhisper?.overallHealth || 'stable'}**`);
  if (report.summary?.autoMergeRecommended) {
    lines.push('');
