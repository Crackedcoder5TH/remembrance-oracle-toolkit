...
class SQLiteStore {
  ...
  _initSchema() {
    ...
    // Schema migration: persist the proven test on entries — the auto-generated
    // test synthesized at submit() time must travel with the entry so it survives
    // round-trips and reaches exports (patterns.json). Without these columns the
    // test text is dropped at the store gate.
    try { this.db.exec(`ALTER TABLE entries ADD COLUMN test_code TEXT`); } catch (e) { if (process.env.ORACLE_DEBUG) console.log('[sqlite:migration] entries.test_code:', e.message); }
    try { this.db.exec(`ALTER TABLE entries ADD COLUMN test_auto_generated INTEGER DEFAULT 0`); } catch (e) { if (process.env.ORACLE_DEBUG) console.log('[sqlite:migration] entries.test_auto_generated:', e.message); }
  ...
  addEntry(entry) {
    ...
    this.db.prepare(`
      INSERT INTO entries (id, code, language, description, tags, author,
        coherency_total, coherency_json, test_passed, test_output, test_code,
        test_auto_generated, validated_at,
    ...
    `).run(
      ...
      entry.testOutput || null,
      entry.testCode == null ? null : String(entry.testCode),
      entry.testAutoGenerated ? 1 : 0,
      now, now, now
    );
  ...
  _rowToEntry(row) {
    const __retVal = {
      ...
      testCode: row.test_code == null ? null : row.test_code,
      testAutoGenerated: row.test_auto_generated === 1,
      validation: {
