  ...
  try {
    ...
    for (const __p of __lre_enginePaths) {
      try {
        ...
        __contribute({ cost: 1, coherence: Math.max(0, Math.min(1, (__retVal.recentRuns || []).reduce((s, r, _, a) => s + (Number(r.score) || 0) / a.length, 0) || 0)), source: 'oracle:swarm-history:getHistorySummary' });
        break;
      } catch (_) { /* try next */ }
    }
  } catch (_) { /* best-effort */ }
